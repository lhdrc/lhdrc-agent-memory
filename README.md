...
技术栈：TypeScript strict + Bun workspaces；权威存储 = md + frontmatter + git；索引 = PGLite。
...
- **文件是真相（D1）**：所有内容存为 md + frontmatter，git 为版本账本；索引 `.dfmemory/pglite/` 可随时删除，`rebuild-index` 全量恢复。
