...
/**
 * BM25 风格相关度查询：simple FTS + 中文 bigram + title/path ILIKE 加权。
...
SELECT * FROM (
  SELECT path, title,
    ...
    + CASE WHEN title ILIKE '%' || $3 || '%' THEN 2.5 ELSE 0 END
    + CASE WHEN path ILIKE '%' || $3 || '%' THEN 1.5 ELSE 0 END
    ) AS score,
    body_text
  FROM pages
...
LIMIT ${Math.max(1, Math.floor(limit))}`;
  ...
  const params: unknown[] = [q, qng, raw, opts.brainId];
