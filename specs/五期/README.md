...
> **分期**：五期主线已完成；下一期见 [`../七期/`](../七期/)（LLM 真路径）。四期（MCP/REST）仍为 **补充期**（→ [`../四期/`](../四期/)）。  
...
| — | P5.9（已迁） | [`P5.9-session-compile.md`](P5.9-session-compile.md) | — | **superseded** → [`../六期/`](../六期/) |
...
P5.1–P5.8 全部验收口令绿 + DoD 勾选 → **五期主线完成**（访问面仍属四期补充期）。  
原 P5.9 已迁 [`../六期/`](../六期/)，不阻塞本口径。
...
| **五期（本目录）** | 架构剩余能力（P5.1–P5.8 **done**；不含扩 dream） |
| [`六期/`](../六期/) | **done**：会话摄入（inbox → compile → L0） |
| [`七期/`](../七期/) | **当前**：LLM 真路径 + 窗口 + 图（Spec ready） |
