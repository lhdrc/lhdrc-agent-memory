...
> **明确不做（本期）**：postgres 真 `vector` + HNSW（**#9**）；把评测 `query_loop` 做成 CLI 默认；配置 CLI **先方案后实现**（见 [`P12-config-cli.md`](P12-config-cli.md)）。
...
| 错误包装给宿主 agent | [`P12-error-to-model.md`](P12-error-to-model.md)（**原则已锁**：给使用记忆的 agent，不给仓内 complete()；编码另开） |
| 看/改配置 CLI + LLM 开关 + Key 提示 | [`P12-config-cli.md`](P12-config-cli.md)（方案，本期不编码） |
| **#9** HNSW | **不做** |
...
| 3 | [`P12-config-cli.md`](P12-config-cli.md) | 配置面方案 |
...
| 失败/降级审计 | [`failure-degrade-audit.md`](failure-degrade-audit.md) 落盘，不改行为 |
| 配置 CLI 方案 | [`P12-config-cli.md`](P12-config-cli.md) proposal，本期不编码 |
